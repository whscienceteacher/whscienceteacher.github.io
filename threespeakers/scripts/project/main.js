...
function CreateTripleToneBuffer(f1,f2,f3,A1,A2,A3,phiOffset1,phiOffset2,phiOffset3,SampleLength)
{
	const audioBuffer = CreateAudioBuffer(SampleLength /* one second long */);
	...
	for (let i = 0, len = sampleData.length; i < len; ++i)
	{
		...
		// volume, which can be uncomfortable. So reduce the volume to
		// 25% linearly, which is about -12dB.
		sampleData[i] = A1*Math.sin(i*phi1 + phiOffset1)*0.25 + A2*Math.sin(i*phi2 + phiOffset2)*0.25 + A3*Math.sin(i*phi3 + phiOffset3)*0.25;
	}
...
export function PlayTriple(F1,F2,F3,A1,A2,A3,phiOffset1,phiOffset2,phiOffset3)
{
	const toneBuffer = CreateTripleToneBuffer(F1,F2,F3,A1,A2,A3,phiOffset1,phiOffset2,phiOffset3, 1/* frequency of note C5 in Hz */);
	PlayAudioBuffer(toneBuffer);
}
